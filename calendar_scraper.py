...
def get_upcoming_skips(skip_calendar: str=SKIP_CALENDAR_ID):
  ...
  try:
    ...
    for event in events:
        ...
        upcoming_skips.append({'name': event['summary'], 'date': start, 'weekday': weekday})
...
def get_target_work_week() -> tuple:
    ...
    if weekday == 6:
    #   If today is Sunday apply the program on this week's work week
        start_date = today + dt.timedelta(days=1)
    else:
    #   If it's any other day of the week, use next week's work week events,
    #   by calculating the days till next monday
    ...
    time_max = end_date.replace(hour=0, minute=0, second=0).isoformat() + 'Z'
